"""
06_export_map_data_schools.py (Modified Copy)
=============================================
...
def build_geojson(vuln: pd.DataFrame, boundaries: gpd.GeoDataFrame, school_counts: pd.DataFrame = None) -> dict:
    """
    ...
    # Ensure Admin2_Geo column for joining
    ...
    # Merge school counts if available
    if school_counts is not None:
        # school_counts should have [Admin2_Geo, school_count]
    ...
    for _, row in vuln.iterrows():
        ...
        properties = {
            ...
            "edu_baseline":  round(float(row["edu_baseline"]), 3) if pd.notna(row["edu_baseline"]) else None,
            "conflict_score": round(float(row["conflict_score"]), 3) if pd.notna(row["conflict_score"]) else None,
...
def build_trends_json(trends: pd.DataFrame) -> list[dict]:
    ...
    # Convert NaN to None for valid JSON (null)
    # We convert to object type first to allow None in float columns
    return trends.astype(object).where(pd.notnull(trends), None).to_dict(orient="records")
...
if __name__ == "__main__":
    ...
    if in_admin2.exists():
        ...
        geojson    = build_geojson(vuln, boundaries, school_counts)
