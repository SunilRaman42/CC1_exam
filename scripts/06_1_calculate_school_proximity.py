...
def main():
    ...
    BOUNDARIES_PATH = 'data/raw/boundaries/BFA_admin2.geojson'
    ...
    # Spatial join to get provinces
    boundaries = gpd.read_file(BOUNDARIES_PATH)
    name_col = next((c for c in boundaries.columns if any(x in c.lower() for x in ["adm2_en", "adm2_name", "name_2", "shapename", "admin2name"])), boundaries.columns[0])
    schools_gdf = gpd.GeoDataFrame(schools, geometry=gpd.points_from_xy(schools.longitude, schools.latitude), crs="EPSG:4326")
    if boundaries.crs != schools_gdf.crs: boundaries = boundaries.to_crs(schools_gdf.crs)
    joined = gpd.sjoin(schools_gdf, boundaries[[name_col, "geometry"]], how="left", predicate="within")
    schools["province"] = joined[name_col].fillna("Unknown")
    ...
    school_provinces = schools['province'].values
    ...
    for i in range(num_schools):
        if v_scores[i] > 0.01:
            final_state.append({
                ...
                "province": str(school_provinces[i]),
